...
@SerialName("square")
@Serializable
data class NGTSquare(
    @SerialName("size") val size : NGTAutoOrLength? = null, 
...
{
    override fun convert() = TSquare(size?.convert().cast(), width?.convert().cast(), height?.convert().cast(), fill?.convert().cast(), stroke?.convert().cast(), radius?.convert().cast(), inset?.convert().cast(), outset?.convert().cast(), body?.convert().cast())
}
